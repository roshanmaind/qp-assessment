...
import listGroceryItemsUseCase from "../../use-cases/admin/list-grocery-items";
...
export default async function listGroceryItemsController(
  httpRequest: any
): Promise<Response> {
  ...
  const { error, groceries, total } = await listGroceryItemsUseCase({
    pagination,
    searchKeyword,
  });
  if (error) {
    logger.debug("listGroceryItemsController: use case error", {
      error,
      httpRequest: httpRequest.body,
    });
    return {
